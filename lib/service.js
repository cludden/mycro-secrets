...
function Secret(vault, config) {
    ...
    this.fetchSecrets = function(cb) {
        const self = this;
        const env = self._getEnv();
        const map = flattenPaths(_.get(self.config, `secrets.envs.${env}`, {}));
...
/**
 * Wrapper function around the current environment
 * @return {String}
 * @private
 */
Secret.prototype._getEnv = function() {
    let env = _.get(this.config, 'hook.env');
    if (!env) {
