...
function Secret(vault, config) {
    ...
    /**
     * Private variable for storing secrets.
     * @type {Object}
     * @private
     */
    const store = {};
        ...
        operation.attempt(function(attemptN) {
            vault.get(path, {
                ...
                if (_.isNumber(res.lease_duration) && res.lease_duration > 0) {
                    const timeout = res.lease_duration * 1000;
                ...
                // update secrets
                if (address === '.') {
                    _.merge(store, res.data);
                } else {
                    if (!_.get(store, address)) {
                        _.set(store, address, res.data);
                    } else {
                        const tempStore = {};
                        _.set(tempStore, address, res.data);
        ...
        async.waterfall([
            ...
            function validateSecrets(fn) {
                ...
                if (hook.validate.length < 2) {
                    ...
                    _.merge(store, validated);
                    return async.setImmediate(fn);
                } else {
                    hook.validate(store, function(err, validated) {
                        if (err) {
                            return fn(err);
                        }
                        _.merge(store, validated);
